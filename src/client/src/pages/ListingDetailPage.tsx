...
export function ListingDetailPage() {
  ...
  const toggleFavorite = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        data.isFavorited ? "Добавена в любими" : "Премахната от любими",
      );
    },
  });
    ...
    return (
      <Container className="py-16 text-center">
        ...
        <p className="mt-2 text-sm text-fg-muted">
          Тази обява може да е премахната или да не съществува.
        </p>
        <Link
          to="/search"
  ...
  const handleSendMessage = () => {
    ...
    window.open(
      `mailto:${listing.sellerEmail}?subject=${subject}&body=${body}`,
    );
            ...
            <Button
              ...
              leadingIcon={
                <Heart
                  className={cn(
                    "h-4 w-4",
                    isFavorited && "fill-danger text-danger",
                  )}
                />
              }
          ...
          <section className="card-shell p-6">
            ...
            <dl className="mt-6 grid grid-cols-2 gap-y-4 sm:grid-cols-3">
              ...
              <Spec
                ...
                value={
                  TRANSMISSION_LABELS[listing.transmissionType] ??
                  listing.transmissionType
                }
              />
              <Spec
                label="Каросерия"
                value={BODY_TYPE_LABELS[listing.bodyType] ?? listing.bodyType}
              />
              <Spec
                label="Задвижване"
                value={
                  DRIVE_TYPE_LABELS[listing.driveType] ?? listing.driveType
                }
              />
              <Spec
                label="Цвят"
                value={COLOR_LABELS[listing.color] ?? listing.color}
              />
              <Spec
                label="Състояние"
              ...
              {listing.engineDisplacementCc != null && (
                <Spec
                  label="Двигател"
                  value={`${listing.engineDisplacementCc} куб.см`}
                />
              )}
              {listing.vinNumber && (
                <Spec label="VIN" value={listing.vinNumber} />
              )}
            </dl>
          </section>
  ...
  if (!analysis) {
    return (
      <span className="text-xs text-fg-subtle">
        Недостатъчно данни за сравнение
      </span>
    );
  }
    ...
    return (
      <div className="space-y-1" title={tooltip}>
        ...
        <p className="text-xs text-fg-subtle">
          спрямо {analysis.sampleSize} обяви
        </p>
      </div>
    );
  }
  ...
  return (
    <section className="card-shell p-6">
      ...
      <ResponsiveContainer width="100%" height={200}>
        ...
        >
          ...
          <XAxis
            ...
            tickFormatter={(i) =>
              history[i]
                ? new Date(history[i].recordedAt).toLocaleDateString("bg-BG", {
                    month: "short",
                    day: "numeric",
                  })
                : ""
      ...
      <p className="mt-3 text-xs text-fg-subtle">
        {history.length} промени · текуща {formatPrice(last)} · първоначална{" "}
        {formatPrice(first)}
      </p>
    </section>
  );
...
function ExternalSellerCard({
  ...
  const e164Digits = normalizedPhone
